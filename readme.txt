...
* More user information - server time for TOTP, error messages when OTP registration fails, ...
* It is now possible to enforce use of OTP only - this is done per user role in plugin settings.
* Improved implementation of software-only random number generator. Plugin will not fail when /dev/random does not exist. 
* Improved use of time - we now use only GMT time as local time is often set incorrectly in php.ini files.
